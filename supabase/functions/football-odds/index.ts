...
function oddsApiTimestamp(value: string) {
  const parsed = new Date(value);
  if (!Number.isFinite(parsed.getTime())) throw new Error(`Invalid odds window timestamp: ${value}`);
...
Deno.serve(async (req: Request) => {
  ...
  try { publishable = JSON.parse(Deno.env.get("SUPABASE_PUBLISHABLE_KEYS") || "{}").default || legacyAnon; } catch { }
  ...
  url.searchParams.set("commenceTimeFrom", oddsApiTimestamp(range.window_starts_at));
  url.searchParams.set("commenceTimeTo", oddsApiTimestamp(range.window_ends_at));
  ...
  if (!provider.ok) {
    const providerText = await provider.text().catch(() => "");
    let providerError: unknown = providerText;
    try { providerError = providerText ? JSON.parse(providerText) : null; } catch { }
    console.error("Odds provider request failed", {
      status: provider.status,
      sportKey,
      commenceTimeFrom: oddsApiTimestamp(range.window_starts_at),
      commenceTimeTo: oddsApiTimestamp(range.window_ends_at),
      providerError,
    });
    return reply({
      error: `Odds provider error ${provider.status}`,
      providerError,
      remaining,
      used,
    }, provider.status);
  ...
  console.log("football_odds_result", JSON.stringify({
    sport, week,
    windowStartsAt: range.window_starts_at,
    windowEndsAt: range.window_ends_at,
    providerGames: Array.isArray(raw) ? raw.length : 0,
    gamesWithSpreads: games.length,
    events: (Array.isArray(raw) ? raw : []).map((game: any) => ({
      commenceTime: game.commence_time,
      awayTeam: game.away_team,
      homeTeam: game.home_team,
      bookmakerCount: game.bookmakers?.length ?? 0,
      spreadMarketCount: (game.bookmakers || []).filter((book: any) =>
        book.markets?.some((market: any) => market.key === "spreads" && market.outcomes?.some((outcome: any) => outcome.point != null))
      ).length,
    })),
  }));
