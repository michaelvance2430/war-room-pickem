...
const SCHOOL_MODIFIERS = new Set(["state", "tech", "central", "eastern", "western", "northern", "southern", "international", "christian", "pine", "bluff"]);
...
async function loadCfbRanks(start: string, end: string) {
  ...
  url.searchParams.set("dates", `${compactDate(start)}-${compactDate(end)}`);
    ...
    for (const event of scoreboard.events || []) {
      for (const competition of event.competitions || []) {
        for (const competitor of competition.competitors || []) {
          ...
          for (const label of [team.displayName, team.shortDisplayName, team.location, team.name, team.abbreviation]) {
...
function rankForTeam(value: string, ranks: Map<string, number>) {
  ...
  const candidates = [...ranks.entries()]
    .filter(([label]) => label.length >= 4 && (team.startsWith(`${label} `) || label.startsWith(`${team} `)))
