...
async function fcmAccessToken() {
  ...
  const claim = base64url(encoder.encode(JSON.stringify({
    iss: required("FCM_CLIENT_EMAIL"), scope: "https://www.googleapis.com/auth/firebase.messaging",
  ...
  try {
    ...
    for (const device of devices ?? []) {
      ...
      if (device.platform === "android") {
        if (!fcmToken) { failures.push("android:FCM_NOT_CONFIGURED"); continue; }
        ...
        if (response.ok) {
          const { error } = await supabase.rpc("record_push_notification_delivery", { p_job_id: job.id, p_device_token: device.device_token });
