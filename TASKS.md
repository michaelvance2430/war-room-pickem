...
- [x] ~~Restore the Fieldhouse Home report rail~~ (2026-09-05)
  - Preserved the simplified upper Home hierarchy for NCAAM and NCAAW, then restored a collapsed More from War Room section with the real Patreon destination plus Field Reports cards for the data-backed Fieldhouse Dispatch, Locker Wire, and support-email workflow. Moved the Fieldhouse Crystal Ball receipt out of the buried Trophy Case position and into Room Operations immediately below Locker Room, matching the shared CFB/NFL profile hierarchy. The isolated preview switcher remains intentionally limited to Fieldhouse sports; CFB/NFL state was not mixed into the preview.
