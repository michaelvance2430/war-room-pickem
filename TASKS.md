...
- [x] ~~Close the Build 21 NFL hardware and Fieldhouse weapon gaps~~ (2026-09-05)
  - Connected the authoritative NFL Final Thirteen and Toilet Bowl receipts to the native postseason command screen. A newly earned receipt now presents its engraved hardware once, remains reopenable from a permanent hardware panel, and is filtered to the signed-in player, exact league, exact season, and championship or Toilet Bowl award types. Upgraded the permanent Weapon Service Record contract to accept canonical NCAAM and NCAAW Hellfire events, including existing-ledger constraint migration. Standardized every regular-season Nuke, JDAM, and Hellfire to 2× all points earned on its permanently sealed card; their risk-based postseason rules remain separate. Full native unit and UI suites plus the focused Build 21 release gates pass; the database remains review-only and production is untouched.

- [x] ~~Restore the Fieldhouse Home report rail~~ (2026-09-05)
  - Preserved the simplified upper Home hierarchy for NCAAM and NCAAW, then restored a collapsed More from War Room section with the real Patreon destination plus Field Reports cards for the data-backed Fieldhouse Dispatch, Locker Wire, and support-email workflow. Moved the Fieldhouse Crystal Ball receipt out of the buried Trophy Case position and into Room Operations immediately below Locker Room, matching the shared CFB/NFL profile hierarchy. Corrected the Picks lane contract so Make Picks is red, Picks Complete is green, missed windows are red, and an unpublished card honestly says Card Pending instead of presenting a neutral Make Picks control. Fieldhouse preview mode now stays on the Fieldhouse profile even when the simulator retains a live login, preventing a saved account with no selected live Fieldhouse membership from silently routing review into the wrong profile. The isolated preview switcher remains intentionally limited to Fieldhouse sports; CFB/NFL state was not mixed into the preview.
...
- [x] ~~Add the Build 21 multi-sport season-closeout authority~~ (2026-09-05)
  - Replaced the CFB-only receipt contract with a review-only sport-aware closeout RPC for CFB, NFL, NCAAM, and NCAAW. It preserves the legacy CFB entry point, supports honest co-champion arrays, requires an Official-season receipt, validates each sport's trophy catalog, and checks final evidence against the correct football or Fieldhouse authority. The existing CFB consumer now uses the generic RPC. Multi-sport, CFB regression, Fieldhouse authority, and targeted lint checks pass; production was inspected read-only and not changed. The native NFL ceremony is now attached in the later Build 21 closeout task above.
