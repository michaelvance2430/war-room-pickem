...
assert(contract.includes("ACCOUNT_LIFECYCLE_PUBLIC = true"), "public gate is not open");
assert(account.includes("ACCOUNT_LIFECYCLE_PUBLIC &&"), "Account entry is not gated");
assert(!route.includes("ACCOUNT_DELETION_ENABLED"), "undeclared server environment gate remains");
...
console.log("[account-deletion-ui] PASS — public entry, Pass the Keys, confirmation, and Foundry repair states verified");
