...
assert.match(clientScores, /liveWindow\) return \{ minAgeSeconds: 180, daysFrom: 1 \}/);
assert.match(autonomousScores, /liveWindow\)return \{minAgeSeconds:180,daysFrom:1\}/);
assert.match(tournamentScores, /hasLiveWindow \? 180 : 900/);
...
console.log("Build 21 Odds API budget PASS - provider score pulls are capped at three minutes per active sport while the native football board keeps its 15-second cache refresh");
