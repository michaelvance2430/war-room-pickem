...
assert.match(content, /BoardGamePanel\(game: game, picks: picks, score: scores\[game\.id\], sportId: sportId\)/);
assert.match(content, /PICKS CLASSIFIED UNTIL KICKOFF/);
assert.match(content, /item\.1\.isBestBet/);
assert.match(content, /Text\(item\.0\)/);
assert.match(content, /SupabaseAPI\.footballScores\(token: token, leagueId: league\.leagueId, sportId: league\.leagues\.sportId, weekNumber: card\.weekNumber\)/);
...
assert.match(scores, /Valid scoring week required/);
assert.match(scores, /hasRequestedWeek \? `week_number=eq\.\$\{week\}` : "order=week_number\.desc"/);
...
assert.match(autonomous, /if\(card\.card_kind==="conference_championship"\)return 4/);
assert.match(autonomous, /if\(\["ncaam","ncaaw"\]\.includes\(sport\)\)return 10/);
assert.match(autonomous, /if\(sport==="cfb"\)[\s\S]*selected>=5&&selected<=10\?selected:-1/);
assert.match(autonomous, /if\(finals\.length<5\|\|finals\.length>10\)return null/);
