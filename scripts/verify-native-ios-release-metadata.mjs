...
assert.deepEqual(new Set(buildNumbers), new Set(["30"]), "Every native target configuration must be stamped Build 30");
...
assert.deepEqual(new Set(marketingVersions), new Set(["3.5"]), "Every native target configuration must be stamped Version 3.5");
...
assert.match(privacy, /NSPrivacyCollectedDataTypeDeviceID/, "notification device identifiers must be disclosed");
assert.ok(existsSync(new URL("../native-ios/WarRoom/Assets.xcassets/AppIcon.appiconset", import.meta.url)));

console.log("Native iOS release metadata PASS - Version 3.5, Build 30, bundle identity, signing team, push/deep-link entitlements, privacy manifest, encryption declaration, and app icon are present");
