...
export default function FoundryPreviewPage() {
  ...
  return <main className="relative z-[1] min-h-screen bg-background/80 pb-32 text-foreground">
    ...
    <PreviewChrome state={state} busy={!!simulation} onWeek={() => isFoundrySeasonFinal(state) ? setRingCeremony(state) : state.sport === "cbb" && state.week >= foundryPostseasonStartWeek("cbb") && !state.ncaaBracketLocked ? setView("postseason") : setSimulation({ kind: "week", step: 0 })} onRegular={() => { if (state.sport === "cfb") localStorage.removeItem("warroom-foundry-cfb-act-three-v3"); setSimulation({ kind: "regular", step: 0 }); }} onSeason={() => isFoundrySeasonFinal(state) ? setRingCeremony(state) : state.sport === "cbb" && state.week >= foundryPostseasonStartWeek("cbb") && !state.ncaaBracketLocked ? setView("postseason") : setSimulation({ kind: "season", step: 0 })} onRole={(role) => update(setFoundryWalkthroughRole(state, role))} onGazette={() => { setGazetteWeek(state.gazetteWeeks[state.gazetteWeeks.length - 1] || null); setView("gazette"); }} onBrackets={() => setView("postseason")} onHome={() => setView("home")} onReset={() => { localStorage.removeItem("warroom-foundry-cfb-act-three-v3"); localStorage.removeItem("warroom-foundry-cfb-act-three-v2"); update(createFoundryWalkthrough(state.sport, 1, state.role)); setGazetteWeek(null); setView("home"); }} />
  </main>;
}
...
function HellfireAuthorizeButton({ count, onAuthorize }: { count: number; onAuthorize: () => void }) {
  return <button type="button" onClick={onAuthorize} className="mt-3 flex min-h-16 w-full items-center justify-center gap-3 rounded-2xl border-2 border-red-500 bg-[repeating-linear-gradient(135deg,#2a0802_0,#2a0802_12px,#050505_12px,#050505_24px)] px-4 text-sm font-black text-orange-100 shadow-[0_0_34px_rgba(249,115,22,.45)]"><WarRoomArsenalIcon kind="hellfire" size={52}/><span><small className="block text-[8px] uppercase tracking-[.2em] text-orange-300">M.A.P.’s · Mutually Assured Picks</small>AUTHORIZE HELLFIRE MODE<small className="mt-1 block text-[8px] text-orange-200/70">Computer assumes command · {count}/67 human picks on file</small></span></button>;
}
