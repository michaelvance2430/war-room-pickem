...
export default function FoundryPreviewPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      const cfbActBreak = next.sport === "cfb" && next.week >= foundryPostseasonStartWeek("cfb");
      setView(cfbActBreak ? "home" : simulation.kind === "week" ? "gazette" : simulation.kind === "regular" ? "postseason" : "home");
...
function Home({ state, go }: { state: FoundryWalkthrough; go: (v: View) => void }) {
  ...
  if (state.sport === "cfb" && state.week >= foundryPostseasonStartWeek("cfb")) return <div className="space-y-4"><section className="grid grid-cols-2 gap-3"><Stat label="Final regular rank" value={`#${rank}`} note={`${me.points} season points`}/><Stat label="Season status" value="ACT III" note="postseason active"/></section><FoundryCfbActThree seasonWeek={state.week} postseasonWeek={foundryPostseasonStartWeek("cfb")} /></div>;
...
function NcaaBracketPicker({ state, onUpdate }: { state: FoundryWalkthrough; onUpdate: (next: FoundryWalkthrough) => void }) {
  ...
  return <section className="rounded-2xl border border-orange-300/40 bg-orange-950/20 p-3 sm:p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-[9px] font-black uppercase tracking-[.18em] text-orange-300">Selection Sunday · 68 teams · 67 decisions</p><h3 className="text-xl font-black">The Bracket Drop</h3><p className="mt-1 text-[10px] text-muted">Fill the entire bracket once. Lock it before the First Four. Then watch the evidence accumulate.</p></div><div className="grid grid-cols-2 gap-1"><div className="rounded-xl border border-orange-300/30 bg-black/30 px-2 py-2 text-center"><strong className="block text-lg text-orange-200">{count}/67</strong><span className="text-[8px] uppercase text-muted">picked</span></div><div className="rounded-xl border border-emerald-300/30 bg-black/30 px-2 py-2 text-center"><strong className="block text-lg text-emerald-200">{score}</strong><span className="text-[8px] uppercase text-muted">points</span></div></div></div>
    {!state.ncaaBracketLocked && resultWindow === 0 && <><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => onUpdate({ ...state, ncaaPicks: generateNcaaPicks(777) })} className="min-h-11 rounded-xl border border-orange-300/40 text-[10px] font-black text-orange-200">Foundry autofill 67</button><button type="button" disabled={count !== 67} onClick={() => onUpdate({ ...state, ncaaBracketLocked: true })} className="min-h-11 rounded-xl bg-orange-300 text-[10px] font-black text-black disabled:opacity-35">Lock entire bracket</button></div><button type="button" disabled={count !== 67} onClick={() => onUpdate(launchFoundryHellfire(state))} className="mt-3 flex min-h-16 w-full items-center justify-center gap-3 rounded-2xl border-2 border-red-500 bg-[repeating-linear-gradient(135deg,#2a0802_0,#2a0802_12px,#050505_12px,#050505_24px)] px-4 text-sm font-black text-orange-100 shadow-[0_0_34px_rgba(249,115,22,.45)] disabled:cursor-not-allowed disabled:border-slate-600 disabled:opacity-45 disabled:shadow-none"><WarRoomArsenalIcon kind="hellfire" size={52}/><span><small className="block text-[8px] uppercase tracking-[.2em] text-orange-300">M.A.P.’s · Mutually Assured Picks</small>{count === 67 ? "AUTHORIZE HELLFIRE MODE" : `HELLFIRE ARMS AFTER THE FULL BRACKET · ${67 - count} PICKS REMAIN`}</span></button><p className="mt-2 text-center text-[9px] font-bold text-orange-200/65">Hellfire replaces your completed human bracket with the computer&apos;s bracket, locks it, and preserves the original as evidence.</p></>}
...
function GuidedBracketPick({ game, index, total, selected, result, bracketLocked, onChoose, onBack, onMap }: { game: NcaaGame; index: number; total: number; selected: string | null; result: string | null; bracketLocked: boolean; onChoose: (team: string) => void; onBack: () => void; onMap: () => void }) {
