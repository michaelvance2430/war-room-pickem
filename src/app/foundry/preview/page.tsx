...
const NAV: { id: View; label: string }[] = [
  ...
  { id: "postseason", label: "Brackets" }, { id: "gazette", label: "The Dispatch" }, { id: "locker", label: "Locker" }, { id: "board", label: "Board" }, { id: "profile", label: "Profile" },
];
...
export default function FoundryPreviewPage() {
  ...
  return <main className="relative z-[1] min-h-screen bg-background/80 pb-32 text-foreground">
    ...
    <div className="mx-auto max-w-3xl px-3 py-4">
      <section className="mb-4 rounded-2xl border border-emerald-400/35 bg-emerald-400/[0.05] p-3" aria-label="Foundry feature previews">
        <p className="text-[9px] font-black uppercase tracking-[.2em] text-emerald-300">Feature previews</p>
        <div className="mt-2 grid grid-cols-3 gap-2">
          {[{ id: "gazette" as View, label: "Dispatch" }, { id: "board" as View, label: "Board" }, { id: "locker" as View, label: "Locker" }, { id: "profile" as View, label: "Profile" }, { id: "picks" as View, label: "Picks" }, { id: "postseason" as View, label: "Brackets" }].map((item) => <button key={item.id} type="button" onClick={() => setView(item.id)} className={`min-h-10 rounded-lg border px-2 text-[10px] font-black ${view === item.id ? "border-emerald-300 bg-emerald-300 text-emerald-950" : "border-emerald-400/20 bg-black/20 text-emerald-100"}`}>{item.label}</button>)}
        </div>
      </section>
...
function Home({ state, go, onUpdate }: { state: FoundryWalkthrough; go: (v: View) => void; onUpdate: (next: FoundryWalkthrough) => void }) {
  ...
  return <div className="space-y-3"><FoundrySeasonCommand state={state}/><FoundryLockClock state={state}/><section className="rounded-2xl border border-primary/35 bg-card p-5"><p className="text-[10px] font-black uppercase tracking-[.2em] text-primary">{meta.cadence} · card open</p><h2 className="mt-2 text-3xl font-black">{meta.weekLabel(state.week)}</h2><p className="mt-2 text-sm text-muted">Five games. Confidence 5 through 1. Lock before the first game begins.</p><button onClick={() => go("picks")} className="mt-4 min-h-12 w-full rounded-xl bg-primary text-sm font-black text-black">Open My Picks</button></section>
...
function FoundryLockClock({ state }: { state: FoundryWalkthrough }) {
  const [now, setNow] = useState(() => Date.now());
  const lockAt = Math.min(...state.games.map((game) => Date.parse(game.kickoffAt)).filter(Number.isFinite));
  useEffect(() => {
    if (!Number.isFinite(lockAt) || lockAt <= now) return;
    const id = window.setInterval(() => setNow(Date.now()), lockAt - now < 60 * 60_000 ? 1_000 : 30_000);
    return () => window.clearInterval(id);
  }, [lockAt, now]);
  if (!Number.isFinite(lockAt)) return null;
  const remaining = Math.max(0, lockAt - now);
  const days = Math.floor(remaining / 86_400_000);
  const hours = Math.floor((remaining % 86_400_000) / 3_600_000);
  const minutes = Math.floor((remaining % 3_600_000) / 60_000);
  const label = new Date(lockAt).toLocaleString(undefined, { weekday: "short", month: "short", day: "numeric", hour: "numeric", minute: "2-digit" });
  return <section className="mb-4 rounded-xl border border-emerald-400/45 bg-black/70 px-3 py-3 text-white" role={remaining ? "timer" : "status"}><div className="flex items-center justify-between gap-3"><div><p className="text-[9px] font-black uppercase tracking-[.18em] text-emerald-300">Mission clock · earliest card kickoff</p><p className="mt-1 text-[10px] font-bold text-white/55">Commissioner card · {label}</p></div><p className="shrink-0 text-2xl font-black tabular-nums">{remaining ? `${days}D ${hours}H ${minutes}M` : "LOCKED"}</p></div></section>;
}

function Standings({ state }: { state: FoundryWalkthrough }) {
  if (state.sport === "cbb") {
    return <Page title="Regional Standings" note="East, West, South, and Midwest each carry their own cut line into the Fieldhouse postseason."><div className="space-y-4"><FoundryCrownAndShame state={state} /><div className="grid gap-4 sm:grid-cols-2">{FIELDHOUSE_REGIONS.map((region) => {
...
function FoundryCrownAndShame({ state }: { state: FoundryWalkthrough }) {
  ...
  const eligible = state.players.filter((player) => player.locked);
  const crown = [...eligible].sort((a, b) => b.weekPoints - a.weekPoints || a.name.localeCompare(b.name))[0];
  const shame = [...eligible].sort((a, b) => a.weekPoints - b.weekPoints || a.name.localeCompare(b.name))[0];
  const week = state.gazetteWeeks.at(-1);
  if (!scored || !crown || !shame) return <section className="overflow-hidden rounded-xl border-2 border-dashed border-border bg-card/60"><div className="border-b border-border/80 bg-card/80 px-4 py-2.5"><h2 className="text-sm font-semibold">Crown &amp; Wall of Shame</h2><p className="mt-0.5 text-[11px] leading-relaxed text-muted">Score the opening week. Nobody has earned glory—or embarrassment—yet.</p></div><div className="grid sm:grid-cols-2"><div className="bg-primary/5 p-4 sm:border-r sm:border-border"><p className="text-[10px] font-bold uppercase tracking-[.15em] text-primary">👑 This week&apos;s crown</p><p className="mt-2 text-lg font-bold">No Crown Yet</p></div><div className="border-t border-border bg-toilet/5 p-4 sm:border-t-0"><p className="text-[10px] font-bold uppercase tracking-[.15em] text-toilet">🧻 Wall of shame</p><p className="mt-2 text-lg font-bold">Nobody… yet</p></div></div></section>;
  return <section className="overflow-hidden rounded-xl border border-border"><header className="flex items-center justify-between gap-2 border-b border-border bg-card px-4 py-2.5"><h2 className="text-sm font-semibold">Crown &amp; Wall of Shame</h2><span className="text-[10px] uppercase tracking-wider text-muted">{week === undefined ? "Latest score" : PREVIEW_SPORTS[state.sport].weekLabel(week)}</span></header><div className="grid sm:grid-cols-2"><article className="bg-primary/5 p-4 sm:border-r sm:border-border"><p className="text-[10px] font-bold uppercase tracking-[.15em] text-primary">🐐 This week&apos;s crown</p><h3 className="mt-2 truncate text-lg font-bold">{crown.name}</h3><p className="mt-0.5 text-sm font-semibold text-primary">{crown.weekPoints} pts</p><p className="mt-2 text-xs text-muted">Biggest haul on the scored card. Tip the cap.</p></article><article className="border-t border-border bg-toilet/5 p-4 sm:border-t-0"><p className="text-[10px] font-bold uppercase tracking-[.15em] text-toilet">🛍️ Wall of shame</p><h3 className="mt-2 truncate text-lg font-bold">{shame.name}</h3><p className="mt-0.5 text-sm font-semibold text-toilet">{shame.weekPoints} pts</p><p className="mt-2 text-xs text-muted">Lowest locked total. Brown paper bag energy.</p></article></div></section>;
}
...
function PreviewChrome({ state, busy, onWeek, onRegular, onSeason, onRole, onGazette, onBrackets, onHome, onReset }: { state: FoundryWalkthrough; busy: boolean; onWeek: () => void; onRegular: () => void; onSeason: () => void; onRole: (r: PreviewRole) => void; onGazette: () => void; onBrackets: () => void; onHome: () => void; onReset: () => void }) {
  ...
  return <div ref={root} className="fixed z-50 w-[92vw] max-w-[360px] rounded-2xl border border-emerald-400/50 bg-emerald-950/95 p-2 shadow-2xl backdrop-blur" style={position ? { left: position.left, top: position.top } : { right: 12, bottom: "max(10px, env(safe-area-inset-bottom))" }}>
    ...
    <div className="mt-2 grid grid-cols-2 gap-2"><button disabled={busy} onClick={onWeek} className="min-h-10 rounded-lg bg-emerald-300 px-2 text-[9px] font-black text-emerald-950 disabled:opacity-40">{busy ? "SIMULATING…" : seasonFinal ? "Ring Ceremony" : "Sim Week"}</button><button disabled={busy || seasonFinal} onClick={onRegular} className="min-h-10 rounded-lg border border-amber-300/50 px-1 text-[9px] font-black text-amber-200 disabled:opacity-40">Sim Regular Season</button></div>
