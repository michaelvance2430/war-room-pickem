  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="grid grid-cols-1 gap-2">
              ...
              {/* Creator-only: badge reveal QA (SAFE NAV: badges restored first) */}
              <div className="rounded-xl border border-emerald-500/40 bg-emerald-500/10 p-3 space-y-2">
                ...
                <p className="text-[11px] text-muted leading-snug">
                  Force a one-shot achievement modal for lifecycle stress
                  tests. Does not change standings. Prefer this over
                  Marilynnsmum&apos;s live House Dragon surprise.
                </p>
                <button
                  type="button"
                  onClick={() => {
                    void import("@/lib/badges").then((b) => {
                      const def = b.getBadgeDef("house_dragon_legendary");
                      if (!def) {
                        setLabLog("❌ house_dragon_legendary def missing");
                        return;
                      }
                      window.dispatchEvent(
                        new CustomEvent("warroom-badge-force-celebrate", {
                          detail: {
                            badges: [{ def, earned: true }],
                          },
                        })
                      );
                      setLabLog(
                        "✅ Forced badge reveal queue (House Dragon QA). Close it, then navigate hard."
                      );
                    });
                  }}
                  className="w-full py-2 rounded-lg border border-emerald-400/40 bg-emerald-500/15 text-xs font-extrabold hover:bg-emerald-500/25"
                >
                  Force House Dragon reveal (QA)
                </button>
                <button
                  type="button"
                  onClick={() => {
                    void Promise.all([
                      import("@/lib/badge-celebration"),
                      import("@/lib/league"),
                    ]).then(([m, league]) => {
                      const uid = league.getSession()?.playerId;
                      if (!uid) {
                        setLabLog("❌ No session");
                        return;
                      }
                      const prev = m.readCelebratedIds(uid);
                      const next = prev.filter(
                        (id: string) =>
                          id !== "house_dragon_legendary" &&
                          !id.startsWith("house_dragon_legendary#")
                      );
                      m.writeCelebratedIds(uid, next);
                      setLabLog(
                        "✅ Cleared House Dragon celebration flag — can force reveal again"
                      );
                    });
                  }}
                  className="w-full py-2 rounded-lg border border-border text-xs font-semibold hover:bg-background"
                >
                  Clear House Dragon celebrated (re-test)
                </button>
              </div>
