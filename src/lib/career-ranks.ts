...
const enlisted = (id: string, abbreviation: string, name: string, achievementPoints: number, seasons = 0, sports = 1): CareerRank => ({ id: `rank_${id}`, abbreviation, name, achievementPoints, seasons, sports, grade: "enlisted" });
const officer = (id: string, abbreviation: string, name: string, achievementPoints: number, seasons: number, sports: number): CareerRank => ({ id: `rank_${id}`, abbreviation, name, achievementPoints, seasons, sports, grade: "officer" });
...
export const CAREER_RANKS: readonly CareerRank[] = [
  ...
  officer("col", "COL", "Colonel", 5200, 8, 3),
  officer("bg", "BG", "Brigadier General", 6500, 9, 4),
  officer("mg", "MG", "Major General", 8000, 10, 4),
  officer("ltg", "LTG", "Lieutenant General", 10000, 12, 4),
  officer("gen", "GEN", "General", 12500, 14, 5),
  officer("field_general", "★★★★★", "Five-Star Field General", 15000, 16, 5),
] as const;
...
export function resolveCareerRank(input: { achievementPoints: number; seasons: number; sports: number }): CareerRankProgress {
  ...
  const unlocked = CAREER_RANKS.filter((rank) => achievementPoints >= rank.achievementPoints && seasons >= rank.seasons && sports >= rank.sports);
