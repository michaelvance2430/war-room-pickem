Warning: truncated output (original token count: 45501)
Total output lines: 5774
...
export async function listScoredWeekNumbers(): Promise<number[]> {
  ...
  const session = getSession();
  if (!session?.leagueId) return [];
  const hit = cacheGet(scoredCache, session.leagueId, LIST_TTL_MS);…9501 tokens truncated…ort("./profile-nav-trace");
...
export async function resetSeasonInCloud(): Promise<ResetSeasonResult> {
  ...
  const result: ResetSeasonResult = {
    ...
    postseasonDeleted:
      typeof row.postseasonDeleted === "number"
        ? row.postseasonDeleted
        : Number(row.postseasonDeleted) || undefined,
  };
