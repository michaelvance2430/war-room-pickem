...
export default function FoundryCfbActThree({ seasonWeek = 16, postseasonWeek = 13 }: { seasonWeek?: number; postseasonWeek?: number }) {
  ...
  if (seasonWeek < postseasonWeek) return <section className="relative overflow-hidden rounded-3xl border border-white/10 bg-[radial-gradient(circle_at_top,rgba(56,189,248,.12),transparent_52%),linear-gradient(180deg,#07111f,#020617)] px-5 py-12 text-center"><div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-transparent via-sky-300 to-transparent"/><p className="text-[10px] font-black uppercase tracking-[.28em] text-sky-300">Phase II · Conference Championships</p><h3 className="mt-4 text-3xl font-black">THE PLAYOFF FIELD IS NOT FINAL</h3><p className="mx-auto mt-3 max-w-sm text-sm leading-relaxed text-muted">Conference titles are still being decided. Bowl Mania and the CFP remain sealed until the selection order is final.</p><div className="mx-auto mt-7 max-w-xs rounded-2xl border border-white/10 bg-black/35 p-4"><p className="text-[9px] font-black uppercase tracking-[.18em] text-muted">Phase III opens</p><strong className="mt-1 block text-xl">After Conference Championships</strong><p className="mt-1 text-xs text-sky-200">{postseasonWeek - seasonWeek} week{postseasonWeek - seasonWeek === 1 ? "" : "s"} away</p></div><p className="mt-6 text-[10px] italic text-white/40">Finish Phase II. Then the bowls and playoff field become official.</p></section>;

  if (!lab.actEntered) return <section className="relative overflow-hidden rounded-3xl border-2 border-amber-300/45 bg-[radial-gradient(circle_at_center,rgba(251,191,36,.22),transparent_58%),linear-gradient(180deg,#1c1304,#030712)] px-5 py-12 text-center shadow-[0_0_70px_rgba(251,191,36,.12)]"><div className="absolute inset-x-0 top-0 h-2 bg-gradient-to-r from-red-500 via-amber-300 to-sky-400"/><p className="text-[10px] font-black uppercase tracking-[.3em] text-amber-300">Phase III · Postseason</p><h3 className="mt-4 text-4xl font-black leading-none">CONFERENCE CHAMPIONSHIPS<br/>ARE COMPLETE</h3><p className="mx-auto mt-4 max-w-sm text-sm leading-relaxed text-amber-100/70">The standings have spoken. The arguments have not. Twenty-five bowls, twelve playoff teams, and one piece of commissioner-selected hardware are waiting behind this door.</p><div className="my-7 flex items-center gap-3"><span className="h-px flex-1 bg-amber-300/30"/><span className="text-xl">✦</span><span className="h-px flex-1 bg-amber-300/30"/></div><button type="button" onClick={() => setLab((current) => ({ ...current, actEntered: true }))} className="min-h-14 w-full rounded-2xl bg-amber-300 text-base font-black text-black shadow-[0_0_30px_rgba(251,191,36,.3)]">ENTER PHASE III →</button></section>;
...
function CfpHandoff({ onReset }: { onReset: () => void }) {
  ...
  return <section className="space-y-4" aria-label="Foundry CFP handoff"><header className="rounded-2xl border border-sky-300/40 bg-[radial-gradient(circle_at_top,#0c4a6e,transparent_60%)] p-5"><p className="text-[9px] font-black uppercase tracking-[.2em] text-sky-300">Separate scoring · same Phase III</p><h3 className="mt-1 text-2xl font-black">Road Through the CFP</h3><p className="mt-2 text-xs text-muted">The Bowl Bankroll is closed. Now fill the fixed 12-team, 11-game playoff bracket. No reseeding.</p></header>
