-- Regular-season scoring receipts. Stored beside postseason receipts so the
-- native client has one UUID-attached scorecard stream for the full season.

alter table public.postseason_scorecards
  drop constraint if exists postseason_scorecards_phase_check;
alter table public.postseason_scorecards
  add constraint postseason_scorecards_phase_check check (
    phase in ('regular_season','bowl_opening','bowl_finale_round1','quarterfinals','semifinals','championship')
  );
...
begin
  ...
  select league_id,user_id,extract(year from now())::integer,week_number,'regular_season',components,total_points,
...
revoke all on function public.write_foundry_regular_scorecards(uuid,integer) from public,anon,authenticated;
